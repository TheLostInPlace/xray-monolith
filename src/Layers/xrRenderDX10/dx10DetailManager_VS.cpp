		...
		for (u32 O = 0; O < objects.size(); O++)
		{
			...
			for (; _vI != _vE; _vI++)
			{
				...
				for (; _iI != _iE; _iI++)
				{
					...
					// MT branch precomputes the scaled matrix (mRotY_calculated); a zero scale never
					// reaches the visible list, so only the alpha gate remains.
					if (Instance.alpha <= 0)
						break;
...
{
	// MT branch: grass shadow passes skip the still/LOD variant, and whole-pass culls replace the
	// legacy per-instance culls (the instance buffer is filled once per frame for all passes).
